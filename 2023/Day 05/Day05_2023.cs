...
public class Day05_2023 : Days<long>
{
    ...
    { 
        ...
        return part2 ? Part2(file, match) : Part1(file, match);
    }
    private long Part1(string[] file, MatchCollection match) 
    {
        ...
        foreach(SeedData i in seeds)
            min = (long)Math.Min(i.Value, min);

        return min;
    }
    private long Part2(string[] file, MatchCollection match) 
    {
        long min = Int64.MaxValue;
        List<SeedData> seeds = new();
